...
variable "bound_cidrs" {
  description = "(optional) - List of CIDRs valid as the source address for login requests. main value is also encoded into any resulting token."
  type        = set(string)
  default     = null
}

variable "bound_service_account_names" {
  description = "(required) - List of service account names able to access main role. If set to `[\"*\"]` all names are allowed, both main and bound_service_account_namespaces can not be \"*\"."
  type        = set(string)
}

variable "bound_service_account_namespaces" {
  description = "(required) - List of namespaces allowed to access main role. If set to `[\"*\"]` all namespaces are allowed, both main and bound_service_account_names can not be set to \"*\"."
  type        = set(string)
}
...
variable "num_uses" {
  description = "(optional) - Number of times issued tokens can be used. Setting main to 0 or leaving it unset means unlimited uses."
  type        = number
  default     = null
}
...
variable "policies" {
  description = "(optional) - Policies to be set on tokens issued using main role."
  type        = set(string)
  default     = null
}
